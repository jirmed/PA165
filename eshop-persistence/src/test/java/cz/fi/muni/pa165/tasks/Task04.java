...
public class Task04 extends AbstractTestNGSpringContextTests {
    ...
    /**
     * Find product with name 'Flashlight'. Make sure to use named parameter,
     * e.g. using semicolon :name and setParameter method on Query object
     */
    @Test
    ...
    /**
     * Write query that returns count of Products, make sure to return only 1
     * number
     */
    @Test
    public void countProducts() {
        Long count = em.createQuery("select count(id) from Product p", Long.class).getSingleResult();

        Assert.assertEquals(count, new Long(4));
    }

    /**
     * Find all products that have some categories assigned (tip: use IS NOT
     * EMPTY operator)
     */
    @Test
    ...
    /**
     * Find all products and eagerly fetch all categories. You must use JOIN
     * FETCH clause
     */
    @Test
    public void findProductsWithCategories() {
        List<Product> found = em.createQuery("select p from Product p left join fetch p.categories where p.categories is not empty", Product.class).getResultList();
    ...
    /**
     * Find count of Products per Color. This means that the result should be
     * List of Object[] objects where each Object[] x is in the following
     * format: x[0] = Color x[1] = Long
     *
     * To do this you need to use Group by and also use ORDER BY to order it by
     * Color.
     */
    @Test
    public void groupByAndOrderBy() {
        List<Object[]> found = em.createQuery("SELECT p.color, count(p.id) from Product p group by color order by color", Object[].class).getResultList();
    ...
    /**
     * Use SELECT NEW to construct ColorCount objects directly in the query. The
     * query will be almost the same as in the previous test groupByAndOrderBy
     * only additional thing here is using of the SELECT NEW.
     */
    @Test
    public void groupByAndOrderBySelectNew() {
        List<ColorCount> colorCounts = em.createQuery("select new cz.fi.muni.pa165.tasks.ColorCount(p.color, count(p.id)) from Product p group by color order by color", ColorCount.class).getResultList();
    ...
    /**
     *
     * Now using SELECT NEW create ProductAndCategory objects. Use LEFT JOIN
     * with ON keyword. Join only those tuples where category name is Kitchen.
     *
     * This is very interesting task because it should help you undrestand how
     * ON keyword differes from standard WHERE. he LEFT JOIN semantics together
     * with ON keyword is fairly complicated. See JPA spec section 4.4.5.2
     */
    @Test
    public void leftJoinCagesWithPets() {
        EntityManager em = emf.createEntityManager();
        List<ProductAndCategory> productAndCategory = em.createQuery("select new cz.fi.muni.pa165.tasks.ProductAndCategory(p,c) from Product p left join p.categories c on c.name=:category order by p.name", ProductAndCategory.class).setParameter("category", "Kitchen").getResultList();
    ...
    /**
     * Find all products using criteria API, see
    ...
    public void criteriaFindProductsWithNonEmptyCategory() {
        ...
        Root<Product> product = query.from(Product.class);
        Expression<List<Category>> categories = product.get("categories");
        query.select(product);
        query.where(cb.isNotEmpty(categories));
