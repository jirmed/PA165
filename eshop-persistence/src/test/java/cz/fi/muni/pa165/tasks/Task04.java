...
public class Task04 extends AbstractTestNGSpringContextTests {
    ...
    public void countProducts() {
        Long count = em.createQuery("select count(p.id) from Product p", Long.class).getSingleResult();
    ...
    public void findProductsAddedYesterday() {
        ...
        List<Product> products = em.createQuery("select p from Product p where p.addedDate = :addedDate", Product.class).setParameter("addedDate", cal.getTime().toInstant().atZone(ZoneId.systemDefault()).toLocalDate()).getResultList();
